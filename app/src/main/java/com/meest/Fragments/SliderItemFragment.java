...
public class SliderItemFragment extends Fragment {
  ...
  @StringRes
  private static ArrayList<String> PAGE_TITLES = new ArrayList<>();
  ...
  private static final int[] PAGE_TEXT =
      new int[] {
          R.string.de, R.string.de, R.string.de, R.string.de
      };
  // prepare all subtitle images arrays
  @SuppressLint("SupportAnnotationUsage")
  @StringRes
  private static ArrayList<String> PAGE_IMAGE = new ArrayList<>();

  // prepare all background images arrays
  @StringRes
  private static final int[] BG_IMAGE = new int[] {
      R.drawable.ad_bg, R.drawable.ad_bg, R.drawable.ad_bg,
      R.drawable.ad_bg
  };
  ...
  public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
    ...
    if (PAGE_TITLES.size() != 0){
      // set page title
      title.setText(PAGE_TITLES.get(position));
      // set page sub title text
      //  titleText.setText(PAGE_TEXT[position]);
      // set page image
      //imageView.setImageResource(PAGE_IMAGE.get(position));

      Glide.with(getActivity())
              .load(PAGE_IMAGE.get(position))
              .into(imageView);
    }
