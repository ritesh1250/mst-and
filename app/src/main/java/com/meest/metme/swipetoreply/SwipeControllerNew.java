...
public class SwipeControllerNew extends ItemTouchHelper.Callback {
    ...
    public SwipeControllerNew(Context context, ISwipeControllerActions swipeControllerActions) {
        mContext = context;
        mSwipeControllerActions = swipeControllerActions;

        mReplyIcon = mContext.getResources().getDrawable(com.capybaralabs.swipetoreply.R.drawable.ic_reply_black_24dp);
        mReplyIconBackground = mContext.getResources().getDrawable(com.capybaralabs.swipetoreply.R.drawable.ic_round_shape);
    }


    public SwipeControllerNew(Context context, ISwipeControllerActions swipeControllerActions, int replyIcon, int replyIconBackground, int backgroundColor) {
        mContext = context;
        mSwipeControllerActions = swipeControllerActions;
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                if (mSwipeBack) {
                    if (Math.abs(mView.getTranslationX()) >= convertToDp(100)) {
                        mSwipeControllerActions.onSwipePerformed(viewHolder.getAdapterPosition(), viewHolder);
