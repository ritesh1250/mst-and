...
public class GlideLoader {
    ...
    void loadProfileImage(String imageUrl, ImageView imageView) {
        if (mContext != null && imageView != null) {
            Glide.with(mContext).load(imageUrl).apply(
                    new RequestOptions().error(
                            R.color.transparent
    ...
    void loadHashTagImage(String imageUrl, ImageView imageView) {
        if (mContext != null && imageView != null) {
            Glide.with(mContext).load(imageUrl).placeholder(R.drawable.ic_dummy_thumb).apply(
                    new RequestOptions().error(
                            R.color.transparent
                    ).priority(Priority.HIGH)
            ).into(imageView);
    ...
    void loadMediaRoundBitmap(Bitmap bitmap, ImageView imageView) {
        if (mContext != null && imageView != null) {
            Glide.with(mContext).load(bitmap).apply(
                    new RequestOptions().centerCrop().placeholder(circularProgressDrawable).error(
                            R.color.transparent
                    ).priority(Priority.HIGH)
            ).into(imageView);
        }
