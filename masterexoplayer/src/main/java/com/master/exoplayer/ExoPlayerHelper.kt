...
class ExoPlayerHelper(val mContext: Context, private val playerView: PlayerView, enableCache: Boolean = false, private val loopVideo: Boolean = false, val loopCount: Int = Integer.MAX_VALUE) :
    LifecycleObserver {
    ...
    init {
        if (mCacheEnabled != enableCache || mDataSourceFactory == null) {
            ...
            builder.setAllocator(DefaultAllocator(true, 100 * 1024 * 1024))
            ...
            if (enableCache) {
                ...
                mDataSourceFactory = CacheDataSourceFactory(simpleCache, mDataSourceFactory, FileDataSourceFactory(), CacheDataSinkFactory(simpleCache, (5 * 1024 * 1024).toLong()),
        ...
        if (progressRequired) {
            durationRunnable = Runnable {
                ...
                if (mPlayer.playWhenReady) {
                    durationHandler.postDelayed(durationRunnable!!, 500L)
                }
            }
        }
    }
